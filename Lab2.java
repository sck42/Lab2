public class Lab2 {
	...
	if (manGoingToStIves) {
		...
		//Total is the narrator plus number of wives, sacks, cats, and kittens. 
	    total = 1 + numWives + numSacksPerWife*(numWives) + numSacksPerWife*(numWives)*(numCatsPerSack) + numSacksPerWife*(numWives)*(numCatsPerSack)*(numKitsPerCat);
		System.out.println("Number going to St.Ives " + total);
	} else {
